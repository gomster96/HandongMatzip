...
export default class RestaurantInfo extends React.Component {
    constructor(props) {
      ...
      // this.selectedRestaurantName = this.props.navigation.getParam('informationOfRestaurant').name;
      this.selectedRestaurant = null;
      this.state = {
        selectedRestaurantData : this.props.navigation.getParam('informationOfRestaurant'),
        // index : this.props.navigation.getParam('index'),
        // heartPress: this.props.navigation.getParam('heartButtonPress'),
    ...
    // HeartButtonPress(){
    //   // this.props.navigation.getParam('heartButtonPress');
    //   if(this.state.selectedRestaurantData.heart == "white" ){
    //     this.state.selectedRestaurantData.heart = "#e64980"
    //   }
    //   else{
    //     this.state.selectedRestaurantData.heart = "white";
    //   }
    //   // console.log(this.props.navigation.getParam('index'));
    //   // console.log(this.state.index);
    //   // console.log(this.state.selectedRestaurantData.heart);
    //   this.state.heartPress(this.state.index);
    //   // this.setState({});
    // }
     ...
     */
    loadData() {
      db.collection('restaurants').doc(this.state.selectedRestaurantData.name).get().then(doc => {
        if(!doc.exists) {
    ...
    render() {
      let heartColor;
      if(this.state.selectedRestaurantData.heart==='white'){
        heartColor='md-heart-empty'
      }else{
        heartColor='md-heart'
      }
      //let heartColor=this.state.selectedRestaurantData.heart ? 'md-heart' : 'md-heart-empty'
      let Rating_Bar = [];
      ...
      if(this.state.network == true){
        this.state.splitMenu = this.state.selectedRestaurantData.menu.split("\t");
      return(
        <SafeAreaView 
            style={{
              flex: 1,
              ...
              <View style={styles.content}>
                <View style={styles.title}>
                  <Text 
                    ...
                    numberOfLines={1}
                  >{this.state.selectedRestaurantData.name}
                  </Text>
                  {/* <Text style={{fontSize : RFPercentage(4), fontFamily: 'hanna', flexWrap:'wrap', width:(Dimensions.get('window').width)/100*70}}>
                    { 아리가또맘마 }
                    {this.state.selectedRestaurantData.name}</Text> */}
                  <Icon
                    name={heartColor}
                    type='ionicon'
                    color='#e64980'
                    //color={this.state.selectedRestaurantData.heart}
                    //color={this.state.selectedRestaurantData.heart}
                    size={RFValue(30)}
                ...
                <View style={styles.icons}>
                  ...
                  <View style={{flexDirection: 'row', flexWrap: 'wrap', width: (Dimensions.get('window').width)/100*80, margin:5}}>
                    <Text style={{flexDirection: 'row', flexWrap: 'wrap', fontSize : RFPercentage(3), fontFamily: 'hanna'}}>
                      {this.state.selectedRestaurantData.location}
                    </Text>
                    <Text style={{fontSize : RFPercentage(3), color:'#e64980', fontFamily: 'hanna'}} onPress={()=>Linking.openURL(this.state.selectedRestaurantData.locationLink)}>
                      (지도보기)
                    </Text>
                  </View>
                </View>
                <View style={styles.icons}>
                  ...
                  {this.state.selectedRestaurantData.phoneNumber}
                  </Text>
                  <Text style={{fontSize : RFPercentage(3), color:'#e64980', fontFamily: 'hanna', margin:5}} onPress={()=>call({number : this.state.selectedRestaurantData.phoneNumber})}>
                    (전화걸기)
                  </Text>
                </View>
                <View style={styles.icons}>
                  ...
                  <View style={{flexDirection: 'row', flexWrap: 'wrap', width: (Dimensions.get('window').width)/100*80, margin:5}}>
                    <Text style={{fontSize : RFPercentage(3), fontFamily: 'hanna'}}>
                      {/* 9:00~22:00 */}
                      {this.state.selectedRestaurantData.hour}
                    </Text>
                  </View>
                </View>
                <View style={styles.image}>
                  <View style={styles.ratingstar}>
                    <Text style={styles.textStyle}>
                      ...
                      {/* 평점 : {(this.state.selectedRestaurantData.totalPoint / this.selectedRestaurantData.starUserCount).toFixed(2)} / 5.00 */}
                      평점 : 0 / 5.00
                    </Text>
                ...
                <View style={styles.menuinfo}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between',alignItems:'center', padding : 5}}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={this.onPressComment.bind(this, this.state.selectedRestaurantData.name)}
                    >
                      <Icon
                        name='comment'
                        type='evilicon'
...
//             iconStyle={{paddingRight:5}}
//           />
//           <View style={{flexDirection: 'row', flexWrap: 'wrap', width: (Dimensions.get('window').width)/100*85}}>
//             <Text style={{flexDirection: 'row', flexWrap: 'wrap', fontSize : RFPercentage(3), fontFamily: 'hanna'}}>
//               {this.state.selectedRestaurantData.location}
//             </Text>
//             <Text style={{fontSize : RFPercentage(3), color:'#e64980', fontFamily: 'hanna'}} onPress={()=>Linking.openURL(this.state.selectedRestaurantData.locationLink)}>
...
//           {this.state.selectedRestaurantData.phoneNumber}
//           </Text>
//           <Text style={{fontSize : RFPercentage(3), color:'#e64980', fontFamily: 'hanna'}} onPress={()=>call({number : this.state.selectedRestaurantData.phoneNumber})}>
//             (전화걸기)
//           </Text>
//         </View>
//         <View style={styles.icons}>
...
//         <View style={styles.menuinfo}>
//           <View style={{ flexDirection: 'row', justifyContent: 'space-between',alignItems:'center', padding : 5}}>
//             <Text style={{fontSize : RFPercentage(3.5), fontWeight : 'bold', fontFamily: 'do-hyeon'}}>
//               메뉴 정보
//             </Text>
//             <TouchableOpacity
//               style={{ backgroundColor: 'white', borderRadius: 5, borderColor: 'pink', borderWidth: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 7, paddingVertical: 7}}
